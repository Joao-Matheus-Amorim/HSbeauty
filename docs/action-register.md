...
| A-009 | P2 | Qualidade | Smoke minimo e E2E browser real ativos no CI, incluindo cenarios de erro principais. | Frontend smoke: `frontend/src/__tests__/smoke/*`; Backend smoke: `backend/test/smoke-routes.test.js`; E2E Playwright: sucesso + erro (`login invalido`, `slot indisponivel`) em `frontend/e2e/*`; CI com `npm run test:smoke` e `npm run test:e2e`. | Expandir para regressao visual basica por screenshot em fluxo publico/admin. |
